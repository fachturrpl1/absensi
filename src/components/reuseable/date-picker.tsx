...
export function DatePicker({
    ...
    const detectPreset = (start: Date, end: Date): string | null => {
        ...
        if (startTime === todayTime && endTime === nowTime) return "today"

        const yesterday = new Date(today)
        yesterday.setDate(yesterday.getDate() - 1)
        const yesterdayEnd = new Date(yesterday)
        yesterdayEnd.setHours(23, 59, 59, 999)
        if (startTime === yesterday.getTime() && endTime === yesterdayEnd.getTime()) return "yesterday"

        const last7Start = new Date(today)
        last7Start.setDate(last7Start.getDate() - 6)
        last7Start.setHours(0, 0, 0, 0)
        if (startTime === last7Start.getTime() && endTime === nowTime) return "last_7_days"
        ...
        if (startTime === thisWeekStart.getTime() && endTime === nowTime) return "this_week"
        ...
        if (startTime === lastWeekStart.getTime() && endTime === lastWeekEnd.getTime()) return "last_week"

        const last2WeeksStart = new Date(today)
        last2WeeksStart.setDate(last2WeeksStart.getDate() - 13)
        last2WeeksStart.setHours(0, 0, 0, 0)
        if (startTime === last2WeeksStart.getTime() && endTime === nowTime) return "last_2_weeks"

        const thisMonthStart = new Date(now.getFullYear(), now.getMonth(), 1)
        thisMonthStart.setHours(0, 0, 0, 0)
        if (startTime === thisMonthStart.getTime() && endTime === nowTime) return "this_month"

        const lastMonthStart = new Date(now.getFullYear(), now.getMonth() - 1, 1)
        lastMonthStart.setHours(0, 0, 0, 0)
        const lastMonthEnd = new Date(now.getFullYear(), now.getMonth(), 0)
        lastMonthEnd.setHours(23, 59, 59, 999)
        if (startTime === lastMonthStart.getTime() && endTime === lastMonthEnd.getTime()) return "last_month"
    ...
    const applyPreset = (preset: string) => {
        ...
        switch (preset) {
            ...
            case "this_week": {
                const dayOfWeek = now.getDay()
                const diff = dayOfWeek === 0 ? 6 : dayOfWeek - 1
                start = new Date(now); start.setDate(start.getDate() - diff); start.setHours(0, 0, 0, 0)
                end = new Date(now); end.setHours(23, 59, 59, 999)
                break
            }
            case "last_7_days": {
            ...
            case "last_week": {
                const lastWeekEnd = new Date(now); lastWeekEnd.setDate(lastWeekEnd.getDate() - lastWeekEnd.getDay()); lastWeekEnd.setHours(23, 59, 59, 999)
            ...
            case "last_2_weeks": {
                start = new Date(now); start.setDate(start.getDate() - 13); start.setHours(0, 0, 0, 0)
                end = new Date(now); end.setHours(23, 59, 59, 999)
                break
            }
            case "this_month": {
            ...
            case "last_month": {
                start = new Date(now.getFullYear(), now.getMonth() - 1, 1); start.setHours(0, 0, 0, 0)
                end = new Date(now.getFullYear(), now.getMonth(), 0); end.setHours(23, 59, 59, 999)
                break
            }
        }

        setTempStartDate(start); setTempEndDate(end)
        const lm = new Date(start); const rm = new Date(lm); rm.setMonth(rm.getMonth() + 1)
        setLeftMonth(lm); setRightMonth(rm); setSelectingStart(true)
    }
            ...
            <DropdownMenuContent align="start" className="w-auto p-0 dark:bg-gray-950 dark:border-gray-800">
                <div className="flex">
                    <div className="w-40 border-r border-gray-200 dark:border-gray-800 p-3 space-y-1">
                        {["today", "yesterday", "this_week", "last_7_days", "last_week", "last_2_weeks", "this_month", "last_month"].map((p) => (
                            <button
                                key={p}
                                className={`w-full text-left px-3 py-2 text-sm rounded capitalize ${selectedPreset === p ? 'bg-black text-white dark:bg-white dark:text-black' : 'hover:bg-gray-100 dark:hover:bg-gray-800 dark:text-gray-300'}`}
                    ...
                    <div className="p-4">
                        ...
                        <div className="flex gap-4">
                            <CalendarPane
                                month={leftMonth}
                                onMoveMonth={moveLeftMonth}
                                onDateClick={(d) => handleDateClick(d, leftMonth, true)}
                                generateCalendarDays={generateCalendarDays}
                                isDateInRange={(d) => isDateInRange(d, leftMonth)}
                                isStartOrEndDate={(d) => isStartOrEndDate(d, leftMonth)}
                            />
                            <CalendarPane
                                month={rightMonth}
                                onMoveMonth={moveRightMonth}
                                onDateClick={(d) => handleDateClick(d, rightMonth, true)}
                                generateCalendarDays={generateCalendarDays}
                                isDateInRange={(d) => isDateInRange(d, rightMonth)}
                                isStartOrEndDate={(d) => isStartOrEndDate(d, rightMonth)}
                            />
                        </div>
...
function CalendarPane({
    ...
    generateCalendarDays: (month: Date) => { day: number; isCurrentMonth: boolean }[]
    isDateInRange: (day: number) => boolean
    isStartOrEndDate: (day: number) => boolean
}) {
    return (
        <div className="w-64">
            <div className="flex items-center justify-between mb-4">
                <button className="p-1 hover:bg-gray-100 dark:hover:bg-gray-800 rounded dark:text-gray-300" onClick={() => onMoveMonth(-1)}>
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="grid grid-cols-7 gap-1 text-center text-xs mb-2">
                {["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"].map((d) => (
                    <div key={d} className="font-semibold p-1 text-gray-900 dark:text-gray-100">{d}</div>
                ))}
            </div>
            <div className="grid grid-cols-7 gap-1 text-center text-sm">
                {generateCalendarDays(month).map((d, i) => {
                    const inRange = d.isCurrentMonth && isDateInRange(d.day)
                    const isEdge = d.isCurrentMonth && isStartOrEndDate(d.day)
                    return (
                        <button
                            key={i}
