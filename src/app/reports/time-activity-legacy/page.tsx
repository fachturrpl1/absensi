...
export default function TimeActivityLegacyPage() {
    ...
    const summaryCards = useMemo(() => {
        const totalHours = filteredData.reduce((sum, a) => sum + a.totalHours, 0)
        const totalSpent = filteredData.reduce((sum, a) => sum + a.totalSpent, 0)
        const avgActivity = filteredData.reduce((sum, a) => sum + a.activityPercent, 0) / (filteredData.length || 1)
        const uniqueMembers = new Set(filteredData.map(a => a.memberId)).size

        return [
            { label: "Total Hours", value: totalHours.toFixed(2) + " h" },
            { label: "Total Spent", value: formatCurrency(totalSpent) },
            { label: "Avg Activity", value: avgActivity.toFixed(0) + "%" },
    ...
    return (
        <div className="px-6 py-4">
            <h1 className="text-xl font-semibold mb-5">Time & activity (Legacy)</h1>
            ...
            <div className="mt-6 bg-white border rounded-lg shadow-sm">
                ...
                <div className="p-4 border-b">
                    <div className="relative max-w-sm">
                        ...
                        <Input
                            placeholder="Search member, project, or client..."
                            value={searchQuery}
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {paginatedData.map((activity, idx) => (
                                ...
                                >
                                    ...
                                    <td className="p-4 font-medium text-gray-900">{activity.memberName}</td>
                                    <td className="p-4 text-gray-600">{activity.projectName || '-'}</td>
                                    <td className="p-4 text-gray-600">{activity.clientName || '-'}</td>
                                    <td className="p-4 text-gray-600">{activity.todoName || '-'}</td>
                                    <td className="p-4 text-right font-medium">{activity.totalHours.toFixed(2)} h</td>
                                    <td className="p-4 text-right">{activity.activityPercent.toFixed(0)}%</td>
                                    <td className="p-4 text-right font-medium">{formatCurrency(activity.totalSpent)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
