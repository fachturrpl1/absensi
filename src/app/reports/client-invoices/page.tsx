...
import { DUMMY_CLIENT_INVOICES, DUMMY_CLIENTS, DUMMY_MEMBERS, DUMMY_TEAMS, ClientInvoice } from "@/lib/data/dummy-data"
import { Button } from "@/components/ui/button"
import { Download, Search, Filter, Layers, ChevronDown, ChevronRight } from "lucide-react"
import { Input } from "@/components/ui/input"
import { useTimezone } from "@/components/providers/timezone-provider"
import { ClientInvoicesFilterSidebar } from "@/components/report/ClientInvoicesFilterSidebar"
...
export default function ClientInvoicesPage() {
    ...
    const [selectedFilter, setSelectedFilter] = useState<SelectedFilter>({ type: "members", all: true, id: "all" }) // Kept for header consistency, though invoicing is client-centric
    ...
    const groupedData = useMemo(() => {
        const groups: Record<string, ClientInvoice[]> = {}
    ...
    const handleExport = () => {
        const headers = ["Issue Date,Invoice Number,Client,Status,Total,Paid Amount,Amount Due"]
    ...
    return (
        <div className="px-6 pb-6 space-y-6">
            <h1 className="text-xl font-semibold">Client invoices report</h1>
                ...
                <div className="flex gap-2">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="outline" className="h-9 gap-2">
                                ...
                                <span className="hidden sm:inline">
                                    Group by: {groupBy === "client" ? "Client" : "Status"}
                                </span>
                            </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="w-[180px]">
                            ...
                            <DropdownMenuSeparator />
                            <DropdownMenuRadioGroup value={groupBy} onValueChange={(v) => setGroupBy(v as "client" | "status")}>
