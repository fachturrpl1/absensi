...
import { DUMMY_PROJECT_BUDGETS, DUMMY_MEMBERS, DUMMY_TEAMS } from "@/lib/data/dummy-data"
...
export default function ProjectBudgetsPage() {
    ...
    const filteredData = useMemo(() => {
        let data = DUMMY_PROJECT_BUDGETS

        // Note: Project budgets often aren't filtered by members/teams directly in this view,
        // but for consistency we might filter by projects associated with selected member/team if we had that mapping.
        // For now, we'll keep it simple or assume all projects are visible unless search is used.
        // Use 'updatedAt' or similar for date range if applicable, but budgets are usually total.
    ...
    const summaryCards = useMemo(() => {
        ...
        const totalBudget = filteredData.reduce((sum, p) => sum + p.totalBudget, 0)
        const totalSpent = filteredData.reduce((sum, p) => sum + p.spentBudget, 0)
        const atRiskOrOver = filteredData.filter(p => p.status !== 'on_track').length
    ...
    return (
        <div className="px-6 py-4">
            <h1 className="text-xl font-semibold mb-5">Project Budgets</h1>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {paginatedData.map((row, idx) => {
                                ...
                                return (
                                    ...
                                    >
                                        <td className="p-4 font-medium text-gray-900">{row.projectName}</td>
                                        <td className="p-4 text-gray-600">{row.clientName}</td>
