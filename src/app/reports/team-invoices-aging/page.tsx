                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {agingData.map((member, idx) => (
                                <tr
                                    key={member.id}
                                    style={{ backgroundColor: idx % 2 === 1 ? '#f1f5f9' : '#ffffff' }}
                                    className="transition-colors custom-hover-row"
                                >
                                    <td className="p-4 font-medium text-gray-900">{member.entityName}</td>
                                    <td className="p-4 text-right">{member.current > 0 ? formatCurrency(member.current) : '-'}</td>
                                    <td className="p-4 text-right text-yellow-600">{member.days30 > 0 ? formatCurrency(member.days30) : '-'}</td>
                                    <td className="p-4 text-right text-orange-600">{member.days60 > 0 ? formatCurrency(member.days60) : '-'}</td>
                                    <td className="p-4 text-right text-red-600">{member.days90 > 0 ? formatCurrency(member.days90) : '-'}</td>
                                    <td className="p-4 text-right font-bold">{formatCurrency(member.total)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
