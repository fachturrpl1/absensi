 ...
 *
 * Integrations Page — Production-ready, SSR-safe, accessible.
 *
 * Architecture:
 ...
 *  - <IntegrationsClient> carries the "use client" boundary.
 *    All interactivity (search, connect/disconnect, loading states) lives here.
 *  - <IntegrationCard> is a pure presentational component inside the client boundary.
 *
 * Assumptions:
 *  - API routes exist at /api/integrations (GET) and /api/integrations/[id] (DELETE)
 *    and /api/integrations/[provider]/authorize (POST/GET for OAuth).
 *    These match the routes documented in README.md.
 *  - Until the API is wired, the server component gracefully falls back to the
 *    static INTEGRATIONS_CATALOG so the page is never blank.
 *  - OAuth-based integrations redirect to the provider; simple toggle integrations
 *    call the REST API directly.
 *  - The Supabase `integrations` table stores connected integrations per org.
 */
...
import IntegrationsClient from "@/components/organization/IntegrationsClient"
import type { IntegrationSection } from "@/types/integration"
...
// ---------------------------------------------------------------------------
const INTEGRATIONS_CATALOG: IntegrationSection[] = [
    {
        title: "Communication",
        items: [
            {
...
// ---------------------------------------------------------------------------
async function getIntegrations(): Promise<IntegrationSection[]> {
    try {
        const baseUrl =
        ...
        if (!res.ok) {
            ...
            console.error(
                `[integrations] API responded ${res.status} — falling back to catalog defaults.`,
                errorBody
            )
...
export default async function IntegrationsPage() {
    const sections = await getIntegrations()

    return (
        <Suspense fallback={<IntegrationsPageSkeleton />}>
            <IntegrationsClient initialSections={sections} />
        </Suspense>
    )
}
