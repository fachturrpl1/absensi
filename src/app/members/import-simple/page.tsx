...
export default function MembersImportSimplePage() {
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (data.fingerPagePreview) {
        ...
        // Simpan statistik email untuk dialog konfirmasi
        setEmailStats({
          withEmail: data.fingerPagePreview.withEmailCount || 0,
          withoutEmail: data.fingerPagePreview.withoutEmailCount || 0,
        })
      } else {
        // Fallback: hitung dari errors jika fingerPagePreview tidak ada
        const emailErrors = summary.errors?.filter((err: any) => {
          const message = typeof err === "string" ? err : err?.message || ""
          return message.includes("tidak memiliki email") || message.includes("Email is required")
        }).length || 0
        
        setEmailStats({
          withEmail: summary.success || 0,
          withoutEmail: emailErrors,
        })
      }
  ...
  const handleNext = () => {
    if (currentStep === 1) {
      ...
      // Hitung ulang statistik dari testSummary jika emailStats belum ada
      let withoutEmailCount = 0
      if (emailStats) {
        withoutEmailCount = emailStats.withoutEmail
      } else if (testSummary) {
        // Hitung dari errors yang terkait email
        const emailErrors = testSummary.errors?.filter((err: any) => {
          const message = typeof err === "string" ? err : err?.message || ""
          return message.includes("tidak memiliki email") || message.includes("Email is required")
        }).length || 0
        withoutEmailCount = emailErrors
      }
      
      // Jika ada row tanpa email, tampilkan dialog konfirmasi
      if (withoutEmailCount > 0) {
        // Pastikan emailStats ter-set untuk dialog
        if (!emailStats && testSummary) {
          const emailErrors = testSummary.errors?.filter((err: any) => {
            const message = typeof err === "string" ? err : err?.message || ""
            return message.includes("tidak memiliki email") || message.includes("Email is required")
          }).length || 0
          ...
          setEmailStats({
            withEmail: testSummary.success || 0,
            withoutEmail: emailErrors,
          })
        }
        setShowConfirmDialog(true)
      } else {
        // Langsung lanjut jika semua row punya email
        setCurrentStep(3)
      }
                        ...
                        <div className="space-y-2 pb-2">
                          ...
                          <Select
                            ...
                            onValueChange={(value) => {
                              ...
                              setEmailStats(null) // Reset email stats ketika sheet berubah
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <Button
                              ...
                              onClick={() => {
                                ...
                                setEmailStats(null) // Reset email stats ketika header row berubah
                                    ...
                                    <Select
                                      ...
                                      onValueChange={(value) => {
                                        if (value === "__UNMAPPED__") {
                                          ...
                                          setEmailStats(null) // Reset email stats ketika mapping berubah
                                        } else {
                                          const newMapping = { ...mapping }
                                          Object.keys(newMapping).forEach((key) => {
            ...
            <AlertDialogHeader>
              ...
              <AlertDialogDescription asChild>
                <div className="space-y-3 pt-2">
                  {emailStats && emailStats.withoutEmail > 0 && (
                    <>
                      <p className="text-sm">
                        Masih ada data yang tidak dapat di-import karena tidak memiliki email.
                      </p>
                      <div className="space-y-2 pt-2 border-t">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-sm font-bold text-green-600 dark:text-green-400">
                            {emailStats.withEmail} row
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-medium text-red-600 dark:text-red-400">
                            Data yang tidak bisa di-import (tidak punya email):
                          </span>
                          <span className="text-sm font-bold text-red-600 dark:text-red-400">
                            {emailStats.withoutEmail} row
                          </span>
                        </div>
                      </div>
                    </>
