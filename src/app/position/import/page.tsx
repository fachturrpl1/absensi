...
// Mapping kolom Excel ke kolom tabel positions
...
export default function PositionsImportPage() {
  ...
  const handleFileSelect = async (selectedFile: File, sheetOverride?: string) => {
    ...
    try {
      ...
      const response = await fetch("/api/position/import/headers", {
        method: "POST",
        body: formData,
      })
      ...
      // Auto-map kolom umum ke field positions
      ...
      DATABASE_FIELDS.forEach((field) => {
        const matchingHeader = data.headers.find((header: string) => {
          ...
          return (
            ...
            (field.key === "title" && (headerLower.includes("title") || headerLower.includes("nama") || headerLower.includes("jabatan") || headerLower.includes("posisi"))) ||
            (field.key === "description" && (headerLower.includes("description") || headerLower.includes("deskripsi"))) ||
            (field.key === "level" && (headerLower.includes("level") || headerLower.includes("tingkat"))) ||
  ...
  const validateMapping = (): boolean => {
    // Minimal Title atau Code harus diisi
    if (!mapping.title && !mapping.code) {
      toast.error("Please map either Title or Code column (at least one is required)")
      return false
    }
    return true
  }
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/position/import/process", {
        method: "POST",
        body: formData,
      })
  ...
  return (
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-6xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-bold">Import Positions</h1>
          <p className="text-muted-foreground">
            Import data posisi/jabatan dari file Excel atau CSV dengan proses bertahap.
          </p>
        </div>
              ...
              <div className="w-80 pr-4 border-r pt-6 pb-6 relative z-10 flex flex-col shrink-0">
                ...
                <div className="pt-6 border-t space-y-4">
                  <Alert className="border-blue-500/50 bg-blue-500/5">
                    ...
                    <AlertDescription>
                      Mapping kolom Excel ke field positions. Minimal Title atau Code harus diisi.
                    </AlertDescription>
                  </Alert>

                  <Button
                    onClick={handleTest}
                    disabled={!file || processing || (!mapping.title && !mapping.code)}
                    variant="outline"
                    className="w-full"
                  >
                    {processing ? (
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex gap-2 pt-2">
                    ...
                    <Button
                      ...
                      disabled={processing || (!mapping.title && !mapping.code)}
                      className="flex-1"
                      size="lg"
                    >
                      {processing ? (
                  ...
                  {importSummary && (
                    <>
                      ...
                      <div className="flex gap-2">
                        <Button onClick={() => router.push("/position")} className="flex-1">
                          Back to Positions Page
                        </Button>
                        <Button
                          variant="outline"
                          onClick={() => {
