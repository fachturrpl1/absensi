...
type RecordActivityOption = "off" | "on"

interface MemberWithSetting {
    id: string
    name: string
    avatar?: string | null
    recordActivity: RecordActivityOption
}
...
export default function RecordActivityPage() {
    ...
    const [globalRecordActivity, setGlobalRecordActivity] = useState<RecordActivityOption>("on")
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            ...
            try {
                ...
                let gOption: RecordActivityOption = "on"
                if (orgSettingsRes.success && orgSettingsRes.data) {
                    if (orgSettingsRes.data.record_activity) {
                        setGlobalRecordActivity(orgSettingsRes.data.record_activity as RecordActivityOption)
                        gOption = orgSettingsRes.data.record_activity as RecordActivityOption
                    }
                }
                ...
                if (membersRes.success && membersRes.data) {
                    ...
                    const mapped = membersRes.data.map(m => {
                        ...
                        return {
                            ...
                            name: m.name,
                            avatar: m.avatarUrl,
                            recordActivity: (settings.record_activity || gOption) as RecordActivityOption
                        }
                    })
                    setMembers(mapped)
                ...
                console.error("Failed to load record activity data", err)
    ...
    const handleGlobalRecordActivityChange = async (value: RecordActivityOption) => {
        if (!organizationId) return

        const previous = globalRecordActivity
        setGlobalRecordActivity(value)
        ...
        try {
            ...
            setGlobalRecordActivity(previous)
            toast.error("Failed to update default")
        }
    }

    const handleMemberRecordActivityChange = async (memberId: string, value: RecordActivityOption) => {
        const originalMember = members.find(m => m.id === memberId)
        if (!originalMember) return

        setMembers(prev =>
            prev.map(member =>
                member.id === memberId ? { ...member, recordActivity: value } : member
            )
        )
    ...
    const tabs: SettingTab[] = [
        { label: "ACTIVITY", href: "/features/settings/Activity", active: true },
        { label: "TIMESHEETS", href: "/features/settings/timesheets", active: false },
        { label: "TRACKING", href: "/features/settings/tracking", active: false },
        { label: "SCREENSHOTS", href: "/features/settings/screenshot", active: false },
    ]

    const sidebarItems: SidebarItem[] = [
        { id: "track-apps-urls", label: "Apps & URLs tracking", href: "/features/settings/Activity" },
        { id: "record-activity", label: "Record activity", href: "/features/settings/Activity/record-activity" },
        { id: "track-apps-urls-detailed", label: "Track apps & URLs", href: "/features/settings/Activity/track-apps-urls" },
        { id: "data-privacy", label: "Data privacy", href: "/features/settings/Activity/data-privacy" },
    ]
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-6 pt-4">
                            <div className="flex items-center gap-2">
                                <h2 className="text-[10px] font-medium text-slate-400 uppercase tracking-[0.2em]">RECORD ACTIVITY</h2>
                                <Info className="h-3.5 w-3.5 text-slate-300" />
                            </div>
                            <p className="text-sm text-slate-500 leading-relaxed max-w-2xl font-light">
                                Control whether activity is recorded for team members while they are tracking time.
                            </p>
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center gap-6">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="flex items-center gap-1 rounded-full border border-slate-200 bg-slate-100 p-1 shadow-inner h-10 overflow-hidden">
                                            <button
                                                onClick={() => handleGlobalRecordActivityChange("off")}
                                                className={`px-5 py-1.5 text-xs font-light rounded-xl transition-all h-full ${globalRecordActivity === "off"
                                            ...
                                            <button
                                                onClick={() => handleGlobalRecordActivityChange("on")}
                                                className={`px-5 py-1.5 text-xs font-light rounded-xl transition-all h-full ${globalRecordActivity === "on"
                                                    ? "bg-white text-slate-900 shadow-sm"
                                                    : "bg-transparent text-slate-400 hover:text-slate-600"
                                                    }`}
                                            >
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="mt-8">
                                ...
                                <div className="hidden sm:grid grid-cols-2 py-3 border-b border-slate-100 px-2">
                                    ...
                                    <span className="text-[10px] font-medium text-slate-400 uppercase tracking-widest text-right">Activity Mode</span>
                                </div>
                                        ...
                                        members.map((member) => (
                                            <div key={member.id} className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between py-5 border-b border-slate-100 last:border-0 hover:bg-slate-50/50 px-2 rounded-xl transition-colors group">
                                                ...
                                                <div className="flex items-center justify-between sm:justify-end gap-6 bg-slate-100 sm:bg-slate-50 rounded-2xl p-1 w-full sm:w-fit shadow-inner h-10 overflow-hidden">
                                                    <span className="text-[10px] font-medium text-slate-400 uppercase tracking-widest sm:hidden pl-3">Record:</span>
                                                    <div className="flex items-center gap-1 h-full">
                                                        <button
                                                            onClick={() => handleMemberRecordActivityChange(member.id, "off")}
                                                            className={`px-5 py-1.5 text-[10px] font-light rounded-xl transition-all uppercase tracking-widest h-full ${member.recordActivity === "off"
                                                        ...
                                                        <button
                                                            onClick={() => handleMemberRecordActivityChange(member.id, "on")}
                                                            className={`px-5 py-1.5 text-[10px] font-light rounded-xl transition-all uppercase tracking-widest h-full ${member.recordActivity === "on"
                                                                ? "bg-white text-slate-900 shadow-sm ring-1 ring-slate-200"
                                                                : "text-slate-400 hover:text-slate-600 hover:bg-white/50"
                                                                }`}
                                                        >
