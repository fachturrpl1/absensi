...
import { DUMMY_TEAM_INVOICES, DUMMY_MEMBERS, DUMMY_TEAMS } from "@/lib/data/dummy-data"
...
export default function TeamInvoicesAgingPage() {
    ...
    const filteredAndAggregatedData = useMemo(() => {
        ...
        data.forEach(invoice => {
            ...
            if (!memberMap.has(invoice.memberId)) {
            ...
            const record = memberMap.get(invoice.memberId)!
    ...
    const handleExport = () => {
        const headers = ["Member Name,0-30 Days,31-60 Days,61-90 Days,Total Outstanding"]
        ...
        link.setAttribute("download", "team_invoices_aging.csv")
    ...
    return (
        <div className="px-6 pb-6 space-y-6">
            <h1 className="text-xl font-semibold">Team invoices aging report</h1>
                                    ...
                                    paginatedData.map((member, index) => (
                                        <tr key={index} className="hover:bg-gray-50">
                                            ...
                                            <td className="p-3 text-right text-gray-900">
                                                {new Intl.NumberFormat('en-US', { style: 'currency', currency: member.currency }).format(member.bucket0to30)}
                                            </td>
                                            <td className="p-3 text-right text-gray-900">
                                                {new Intl.NumberFormat('en-US', { style: 'currency', currency: member.currency }).format(member.bucket31to60)}
                                            </td>
                                            <td className="p-3 text-right text-gray-900">
                                                {new Intl.NumberFormat('en-US', { style: 'currency', currency: member.currency }).format(member.bucket61to90)}
                                            </td>
                                            <td className="p-3 text-right font-bold text-gray-900">
                                                {new Intl.NumberFormat('en-US', { style: 'currency', currency: member.currency }).format(member.totalOutstanding)}
                                            </td>
                                        </tr>
                                    ))
                                )}
            ...
            <TeamInvoicesAgingFilterSidebar
